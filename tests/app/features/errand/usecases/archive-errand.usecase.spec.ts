...
describe("Archive Errand Usecase", () => {
    beforeAll(async () => {
    ...
    const createSut = () => {
        return new archiveErrandUsecase();
    };
    ...
    test("deveria retornar sucesso caso o recado seja arquivado", async () =>{
        ...
        expect(result.message).toEqual("Errand successfully archived");
        expect(result).toHaveProperty("data");
    })

    test("deveria retornar falha caso o recado já esteja arquivado", async () =>{
        const sut = createSut();
        const archived = errandMockSut.archived = true;
        ...
        expect(result.message).toEqual("Bad request: Errand already archived");
    })
});
